...
Therefore, we need to scale our data. We scale these by finding the mean of each statistic and finding out how many standard deviations away from the mean each observation is. In statistics, this is also known as finding the Z-Score.
...
To evaluate the accuracy of these models, we'll use R-Squared values. R-Squared is a value that tells us what percent of the variance in the data is captured by our model.

These plots help us to visualize how much variance is captured by the model as well. The two men's models are roughly equally accurate, with both having R-Squared values around 0.75. However, when comparing the two women's models the difference is much more stark. The model that used the half weights has a much lower R-Squared value at 0.6736, while the full-weight model has the highers R-Squared value of the four at 0.8793.
